...
class BacktestApp:
    ...
    def load_data(self):
        ...
        try:
            ...
            self.cap_filter['values'] = ["All"] + sorted(self.master_df['marketcapname'].dropna().unique().tolist())
            self.sector_filter['values'] = ["All"] + sorted(self.master_df['sector'].dropna().unique().tolist())
            self.apply_filter()
